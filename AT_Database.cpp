...
#define _MD_DATABASE_VERS_ "V0.1md"
  ...
  String  AT_GetLocalTime()
    {
      ...
      sprintf(sttime,"%02d.%02d. %02d:%02d:%02d ", day(), month(), hour(), minute(), second());
          #ifdef NUSED //defined(ESP32)
            struct tm timeinfo;
          ...
          while (f.available() && (j<ATWIDGETSPERPAGE))
            {
              if (old)
              ...
              else
                {
                  ...
                  //S3VAL(" status ", i, j, tmp);
              _pages[i].widgets[j].status = tmp.toInt();
              tmp = f.readStringUntil('\n');
                  //S3VAL(" source ", i, j, tmp);
      ...
      for (uint8_t pg = 0; pg<ATMAXPAGES; pg++) {
        for(uint8_t slt = 0; slt < ATWIDGETSPERPAGE; slt++){
          ...
          if ((src >= minSrc) && (src < maxSrc))
            { _pages[pg].widgets[slt].status = ATWIDGET_UNUSED; }
        }
      }
    }
...
// devices
      ...
      while (f.available() && (i<ATMAXDEVICE))
        {
          ...
          tmp = f.readStringUntil('\n');
          Serial.print(i); Serial.print(" "); Serial.println(tmp);
  ...
  boolean   AT_Database::clearDevices()
    { //getWidget returns the widget in slot on page
      for (uint8_t i = 0; i<ATMAXDEVICE; i++)
        { deleteDevice(i); }
      if (writeConfig())
        {
          return writeDevices();
  ...
  String        AT_Database::getValueString(uint16_t index, uint8_t precision, boolean useunit )
    {
      ...
      if (useunit)
        { result += " "+AT_getUnitString(data.unit); }
      return result;
    }
...
// check data
  ...
  // wifi
        ...
        boolean AT_Database::setupWiFi(ATSETUP* psetup, md_wifi* pwifi)
          {
            _psetup = psetup;
            _pwifi  = pwifi;
              //S2VAL(" useWlan refresh ", _psetup->useWlan);
              //S3VAL(" SSID PW NTP ", _psetup->SSID, _psetup->password, _psetup->NTPserver);
              //SVAL(" refresh ");
              //SVAL(" pwifi ", (uint32_t) _pwifi)
            return true;
          }
      #else
        boolean AT_Database::readSetup(String fileName)
          {
            ...
            File f = SPIFFS.open(fileName, "r");
            Serial.println("setup file is open");
            if (!f)
              {
                return false;
              }
            ...
            Serial.println("subend erreicht");
          }
